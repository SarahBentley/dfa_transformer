...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, attn_mask=None, use_flash=True):
        ...
        if self.flash and use_flash: 
        ...
        else: # don't use flash when we want access to attention weights for visualizations.
            ...
            self.attention_weights = att # store attention weights
...
class Block(nn.Module):
    ...
    def forward(self, x, attn_mask=None, use_flash=True):
        x = x + self.attn(self.ln_1(x), attn_mask, use_flash)
...
class GT(nn.Module):
    ...
    def create_attn_mask(self, idx, t):
        ...
        # Create causal mask (ensures attention is only to the left in the input sequence)
        causal_mask = torch.tril(torch.ones(t, t, device=device)).bool()
        mask = padding_mask & causal_mask # (B, 1, 1, T) & (1, 1, T, T) -> (B, 1, T, T)
        return mask


    def forward(self, idx, targets=None, use_flash=True):
        device = idx.device
        ...
        for block in self.transformer.h:
            x = block(x, attn_mask=attn_mask, use_flash=use_flash)

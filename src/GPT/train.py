...
from .inference_GPT import infer, metrics_by_length
...
def create_output_dir():
    ...
    output_dir = os.path.join(current_dir, "GPT_outputs")
...
def generate_run_name(args):
    ...
    run_name = f"run_{current_time}_GPT_{args.DFA}_{args.name}_lr{args.lr}_s{args.max_seq_len}"
    return run_name
...
def get_args():
    ...
    parser.add_argument("--trigram", type=bool, default=False, help="Whether to only put attention on the last three elements of the input")


    parser.add_argument("--gen", type=int, default=30, help="Max sequence length for which we want to test model generalization")
...
def main(args):
    ...
    wandb.init(project='GPT_' + args.DFA + '_' + args.name, config=args)
    config = wandb.config
    ...
    DFA = choose_DFA(args)
    dataloader = DFADataloader(DFA, max_seq_len=args.max_seq_len, pad_idx=0)
